...
--8. 검색 직원 전체 조회 쿼리문 (사번/이름/부서/직위/급여 내림차순 정렬)
-- 사번, 이름, 주민번호, 입사일, 지역, 전화번호
-- , 부서, 직위, 기본급, 수당, 급여

SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE       -- 오라클에서는 세션따라 다르게 보임. 문자열로 바꿔서 구성해서 전달되어야 함.
...
-- 한 줄 구성
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW ORDER BY PAY DESC
;
         
         

-- 검색직원 리스트 출력 쿼리문 (사번/이름/부서/직위)
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE, 'YYYY-MM-DD') AS IBSADATE       -- 오라클에서는 세션따라 다르게 보임. 문자열로 바꿔서 구성해서 전달되어야 함.
     , CITY_LOC, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY
FROM EMPVIEW
WHERE EMP_ID = 1001;
...
-- 이름
SELECT E.EMP_ID, E.EMP_NAME,B.BUSEO_NAME, J.JIKWI_NAME
FROM (SELECT * FROM TBL_EMP WHERE EMP_ID = '1060') E
    , TBL_CITY C, TBL_BUSEO B, TBL_JIKWI J
WHERE E.CITY_ID = C.CITY_ID
  AND E.BUSEO_ID = B.BUSEO_ID
  AND E.JIKWI_ID = J.JIKWI_ID;
...
FROM (SELECT * 
      FROM TBL_EMP 
      WHERE BUSEO_ID = (SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부')) E
    , TBL_CITY C, TBL_BUSEO B, TBL_JIKWI J
WHERE E.CITY_ID = C.CITY_ID
  AND E.BUSEO_ID = B.BUSEO_ID
  AND E.JIKWI_ID = J.JIKWI_ID;
...
FROM (SELECT * 
      FROM TBL_EMP 
      WHERE JIKWI_ID = (SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '사원')) E
    , TBL_CITY C, TBL_BUSEO B, TBL_JIKWI J
WHERE E.CITY_ID = C.CITY_ID
  AND E.BUSEO_ID = B.BUSEO_ID
  AND E.JIKWI_ID = J.JIKWI_ID;
...
UPDATE TBL_EMP
SET EMP_NAME = '선혜연', SSN = '951102-2234567', IBSADATE = TO_DATE('2020-02-12', 'YYYY-MM-DD')
  , CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_LOC = '경기') 
  , TEL = '010-1111-1111'
